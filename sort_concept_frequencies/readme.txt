...
On unix systems the terminal program "tree" facilitates the use of sort_concept_frequencies by calling it on the app folder. Tree views the content of all folder so the selection files can easily be copied and pasted to the command prompt. Example for calling tree in the app folder (to select the current folder type . ) :
